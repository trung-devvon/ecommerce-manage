...
const Text = ({ text = 'A' }) => {
  ...
  const [textInit, setTextInit] = useState(text);
  const [completed, setCompleted] = useState(false);ƒ
  ...
  const drawTemplateLetter = (isCompleted) => {
    ...
    const letterConfig = TextCanvas[textInit];
  ...
  return (
    <div className="flex flex-col items-center gap-4 p-4">
      <div className="flex gap-2 mb-4">
        {Object.keys(TextCanvas).map(letter => (
          <button
            key={letter}
      ...
      <div className="mt-4 flex flex-col items-center gap-2">
        ...
        {completed && (
          <div className="text-sky-500 font-bold text-lg">
            Chúc mừng! Bạn đã hoàn thành chữ {text}
          </div>
        )}
